...
ASMap constructs genetic linkage maps using the MSTmap algorithm, which
clusters markers into linkage groups and orders the markers within each
group in a single pass. This article builds a map from scratch and
points at the rest of the documentation.
...
The package datasets are not lazy-loaded, so they need an explicit
[`data()`](https://rdrr.io/r/utils/data.html) call. `mapDHf` is an
unconstructed doubled haploid wheat marker set, in the layout the
construction function expects: **markers in rows, genotypes in
columns**, with marker names in the `rownames` and genotype names in the
`names`.
...
re-orders the markers within them. Two settings are worth committing to
memory:
