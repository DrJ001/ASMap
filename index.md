...
**ASMap** is a linkage map construction, manipulation and diagnosis
toolkit for biparental populations, built around the efficient
**MSTmap** algorithm of [Wu *et al.*
(2008)](https://doi.org/10.1371/journal.pgen.1000212). MSTmap uses the
minimum spanning tree of a graph to cluster markers into linkage groups
**and** optimally order the markers within each group, in a single pass.

| Pillar | Functions | Description |
|----|----|----|
| Construction | [`mstmap()`](https://drj001.github.io/ASMap/reference/mstmap.cross.md) | Simultaneous linkage group clustering, optimal marker ordering and genetic distance estimation, for `data.frame` or R/qtl `cross` input |
| Marker pull/push | [`pullCross()`](https://drj001.github.io/ASMap/reference/pullCross.md) • [`pushCross()`](https://drj001.github.io/ASMap/reference/pushCross.md) | Temporarily set problematic markers aside before construction and reintroduce them afterwards, rather than discarding them |
| Diagnosis | [`profileGen()`](https://drj001.github.io/ASMap/reference/profileGen.md) • [`profileMark()`](https://drj001.github.io/ASMap/reference/profileMark.md) • [`heatMap()`](https://drj001.github.io/ASMap/reference/heatMap.md) • [`alignCross()`](https://drj001.github.io/ASMap/reference/alignCross.md) | Numerical and graphical interrogation of genotypes, markers and intervals, before and after construction |

Objects use the **R/qtl** `cross` format throughout, so ASMap and
[qtl](https://CRAN.R-project.org/package=qtl) functions can be used
synergistically on the same object at any stage.

Map construction with ASMap follows the same three-stage workflow:
**pre-construction** diagnosis and marker curation, **construction**,
and **post-construction** diagnosis and refinement. The three parts
below mirror that sequence.
...
Doubled haploid populations read in with
[`read.cross()`](https://rdrr.io/pkg/qtl/man/read.cross.html) inherit
class `"bc"`; assigning class `"dh"` gives equivalent construction
results. `"RILn"` populations are imported as `"f2"` and must be
converted with `qtl::convert2bcsft(F.gen = n, BC.gen = 0)`.

### Performance

Full construction from scratch - clustering, ordering and distance
estimation (`bychr = FALSE`):
...
_(Single run, R 4.4.3 on 64-bit Windows. Indicative only - timings depend on the machine, population type and marker data.)
...
[gtools](https://CRAN.R-project.org/package=gtools). All are available
from CRAN.

No external software or licence is needed. The MSTmap C++ source code is
bundled with the package and compiled on installation.
...
| Article | Description |
|----|----|
| [Linkage map construction with ASMap](https://drj001.github.io/ASMap/articles/ASMap.html) | A short introduction: construction from marker scores, the choice of `p.value`, diagnosis of genotypes and markers, and setting markers aside. Also installed with the package as [`vignette("ASMap")`](https://drj001.github.io/ASMap/articles/ASMap.md). |
...
A linkage map should not be built by throwing raw marker data at a
construction algorithm. The functions below support a pre-construction
checklist: check missing values across genotypes and markers, check for
genetic clones, check for excessive segregation distortion, and set
aside markers that are co-located or otherwise problematic.

### Genotype statistics - `statGen()` and `profileGen()`

[`statGen()`](https://drj001.github.io/ASMap/reference/statGen.md)
returns per-genotype statistics;
[`profileGen()`](https://drj001.github.io/ASMap/reference/profileGen.md)
plots the same statistics as a multi-panel lattice display and returns
them invisibly.
...
| `xo.lambda` | [`profileGen()`](https://drj001.github.io/ASMap/reference/profileGen.md) only. Expected genome-wide recombination rate; genotypes whose crossover count significantly exceeds it are annotated on the plot |
| `...` | [`profileGen()`](https://drj001.github.io/ASMap/reference/profileGen.md) only. Passed to the high level lattice plot |

When `xo.lambda` is supplied, each genotype’s recombination count is
tested against that expected rate with a one-tailed Poisson test, and
lines significant at a Bonferroni-adjusted 0.05 level are flagged. The
returned object gains a logical `xo.lambda` element, which pairs
directly with
[`subsetCross()`](https://drj001.github.io/ASMap/reference/subsetCross.md)
to drop the offenders.

### Genetic clones - `genClones()` and `fixClones()`

Highly related or duplicated individuals inflate segregation distortion,
so they should be resolved before construction.
...
| `gc` | [`fixClones()`](https://drj001.github.io/ASMap/reference/fixClones.md) only. The data frame of clone information, normally `genClones()$cgd` |
| `consensus` | [`fixClones()`](https://drj001.github.io/ASMap/reference/fixClones.md) only. If `TRUE` (default), a consensus genotype is formed for each clonal group by intelligently collapsing alleles; if `FALSE`, only the group member with the fewest missing alleles is retained |

[`genClones()`](https://drj001.github.io/ASMap/reference/genClones.md)
extends
[`qtl::comparegeno()`](https://rdrr.io/pkg/qtl/man/comparegeno.html) by
returning breakdown statistics for each reported pair in `$cgd`,
including a `group` column identifying the clonal group. **Inspect
`$cgd` before acting on it** - pairs with few co-scored markers are weak
evidence of cloning and are best removed from the table before calling
[`fixClones()`](https://drj001.github.io/ASMap/reference/fixClones.md).
[`fixClones()`](https://drj001.github.io/ASMap/reference/fixClones.md)
can be used at any stage, as it retains linkage group and marker
...
| `cross` | An R/qtl `cross` object inheriting `"bc"`, `"dh"`, `"riself"` or `"bcsft"` |
| `chr` | Character vector of linkage group names used to subset the map |
| `stat.type` | `"marker"` and/or `"interval"`. [`profileMark()`](https://drj001.github.io/ASMap/reference/profileMark.md) also accepts individual statistics: marker - `"seg.dist"`, `"miss"`, `"prop"`, `"dxo"`; interval - `"erf"`, `"lod"`, `"dist"`, `"mrf"`, `"recomb"` |
| `map.function` | `"kosambi"` (default), `"haldane"`, `"morgan"` or `"cf"`, used for interval statistics |
| `use.dist` | [`profileMark()`](https://drj001.github.io/ASMap/reference/profileMark.md) only. If `TRUE` (default) markers are positioned by actual map distance, otherwise equidistantly |
| `crit.val` | [`profileMark()`](https://drj001.github.io/ASMap/reference/profileMark.md) only. Set to `"bonf"` to annotate markers whose segregation distortion p-value, and intervals whose test of no linkage, fall below a Bonferroni-adjusted 0.05 level |
| `display.markers` | [`profileMark()`](https://drj001.github.io/ASMap/reference/profileMark.md) only. Display marker names on the bottom axis. Default `FALSE` |
| `mark.line` | [`profileMark()`](https://drj001.github.io/ASMap/reference/profileMark.md) only. Draw vertical lines at marker positions. Default `FALSE` |
| `...` | [`profileMark()`](https://drj001.github.io/ASMap/reference/profileMark.md) only. Passed to the high level lattice plot |

`"marker"` statistics use
[`qtl::geno.table()`](https://rdrr.io/pkg/qtl/man/geno.table.html) to
give segregation distortion, allele and missing value proportions, and
double crossover counts at each marker under the current order.
`"interval"` statistics give the estimated recombination fraction and
LOD score between adjacent markers, map distances, and actual
recombination counts.

### Choosing `p.value` - `pValue()`

The `p.value` argument of
[`mstmap()`](https://drj001.github.io/ASMap/reference/mstmap.cross.md)
governs how markers are split into linkage groups, and the appropriate
value depends strongly on population size - larger populations require a
smaller `p.value`.
[`pValue()`](https://drj001.github.io/ASMap/reference/pValue.md) plots
the relationship so an informed choice can be made.
...
Pruning before construction is common, and usually permanent.
[`pullCross()`](https://drj001.github.io/ASMap/reference/pullCross.md)
instead *removes markers to a holding element* of the cross object,
retaining everything needed to push them back later.
...
| `pars` | List of threshold parameters; `NULL` (default) calls [`pp.init()`](https://drj001.github.io/ASMap/reference/pp.init.md) with its defaults |
...
| Parameter | Role in [`pullCross()`](https://drj001.github.io/ASMap/reference/pullCross.md) | Default |
|----|----|----|
...
| `max.rf` | Maximum recombination fraction used when clustering markers back in ([`pushCross()`](https://drj001.github.io/ASMap/reference/pushCross.md)) | `0.25` |
| `min.lod` | Minimum LOD score used when clustering markers back in ([`pushCross()`](https://drj001.github.io/ASMap/reference/pushCross.md)) | `3` |

For `type = "co.located"` the first marker of each co-locating group is
retained in the map as a reference marker and the rest are set aside,
keyed to it.
...
Both construction functions are methods of the generic
[`mstmap()`](https://drj001.github.io/ASMap/reference/mstmap.cross.md)
and share the complete set of MSTmap algorithm parameters. They differ
in their input: a raw data frame of marker scores, or an existing R/qtl
`cross` object.

### From a data frame - `mstmap.data.frame()`

Use this when marker data has not yet been imported into an R/qtl
object. The format follows the marker file required by the standalone
MSTmap software: **markers in rows, genotypes in columns**, marker names
in `rownames`, genotype names in `names`, and every column of class
`"character"` (not factors).
...
Allele coding is strict. For `"BC"`, `"DH"` and `"ARIL"` the two alleles
must be `"A"`/`"a"` and `"B"`/`"b"`; `"ARIL"` additionally assumes
heterozygotes have been set to missing. For `"RILn"`, phase-unknown
heterozygotes must be coded `"X"`. Missing scores are `"U"` or `"-"` for
all population types.

### From an R/qtl cross object - `mstmap.cross()`

The more flexible of the two, and the recommended entry point. It
accepts a constructed *or* unconstructed cross object and can rebuild
the whole map or re-order markers within nominated linkage groups.
...
The single most informative check on a constructed map.
[`heatMap()`](https://drj001.github.io/ASMap/reference/heatMap.md)
displays pairwise estimated recombination fractions and pairwise LOD
scores of linkage **with a separate legend for each**, so the two scales
can be tuned until their heat matches. Consistent heat within linkage
groups indicates strong linkage between nearby markers; blocks of shared
heat between groups indicate groups that should be merged.
...
| `...` | Additional arguments, largely passed to [`fields::image.plot()`](https://rdrr.io/pkg/fields/man/image.plot.html). **Avoid `legend.args`**, which is used internally |

### Linkage group identity and orientation - `alignCross()`

Newly constructed linkage groups carry arbitrary names and orientation.
[`alignCross()`](https://drj001.github.io/ASMap/reference/alignCross.md)
plots marker positions against one or more reference maps to identify
which group is which and whether it needs inverting - a positive slope
indicates correct orientation, a negative slope indicates the group
should be reversed.
...
Data frame elements of `maps` must contain the columns `"marker"`,
`"ref.chr"` and `"ref.dist"`, or an error is produced.

### Reintroducing markers - `pushCross()`

The complement of
[`pullCross()`](https://drj001.github.io/ASMap/reference/pullCross.md).
Markers held aside are pushed back into the established map, then the
map is re-ordered by linkage group.
...
The threshold logic is **deliberately the reverse** of
[`pullCross()`](https://drj001.github.io/ASMap/reference/pullCross.md):

| Parameter | Role in [`pushCross()`](https://drj001.github.io/ASMap/reference/pushCross.md) | Default |
|----|----|----|
...
So markers are typically pulled at one threshold and only pushed back if
they clear a more permissive one, letting a marker earn its place in the
map.

`type = "unlinked"` is the mechanism for pushing new markers into an
established map - the basis of the fine mapping workflow described in
the articles. For all types other than `"co.located"`, markers are
allocated to linkage groups by a fast clustering method that reduces the
map to a skeleton marker set, tuned via `max.rf` and `min.lod`. Avoid
`"UL"` in linkage group names, as it is used internally for unlinked
groups.

> [`pushCross()`](https://drj001.github.io/ASMap/reference/pushCross.md)
> allocates markers to linkage groups but does **not** re-construct the
> map. Follow it with `mstmap(..., bychr = TRUE, p.value = 2)` to place
> the new markers optimally.
...
|----|----|
| [`breakCross()`](https://drj001.github.io/ASMap/reference/breakCross.md) | `split` - a list named by linkage group containing the marker names *immediately preceding* each split point. `suffix` - `"numeric"` or `"alpha"`, or a list of explicit new names. `sep` - separator between old name and suffix, default `"."` |
| [`mergeCross()`](https://drj001.github.io/ASMap/reference/mergeCross.md) | `merge` - a list whose elements contain the linkage groups to merge, each element named by the proposed new linkage group name. `gap` - cM gap inserted between merged elements, default `5` |
| [`combineMap()`](https://drj001.github.io/ASMap/reference/combineMap.md) | `...` - any number of cross objects, all of identical class. `id` - common genotype column, default `"Genotype"`. `keep.all` - retain genotypes absent from some maps, default `TRUE`. `merge.by` - `"genotype"` (default) or `"marker"` |
| [`subsetCross()`](https://drj001.github.io/ASMap/reference/subsetCross.md) | `chr` - logical, numeric or character vector of linkage groups. `ind` - logical or numeric vector of individuals |

Two cautions worth repeating:

- **Use
  [`subsetCross()`](https://drj001.github.io/ASMap/reference/subsetCross.md),
  not `subset.cross()`**, whenever the object carries `"co.located"`,
  `"seg.distortion"` or `"missing"` elements from a
  [`pullCross()`](https://drj001.github.io/ASMap/reference/pullCross.md)
  call.
  [`subsetCross()`](https://drj001.github.io/ASMap/reference/subsetCross.md)
  subsets those elements too and recalculates their statistics;
...
- [`combineMap()`](https://drj001.github.io/ASMap/reference/combineMap.md)
  applies **no consensus map algorithm**. Chromosome identity and
  genetic distances are taken from the first appearance of a marker
  across the maps supplied, and the result is not re-constructed - run
  [`mstmap()`](https://drj001.github.io/ASMap/reference/mstmap.cross.md)
  afterwards.
...
| `map.function` | `"kosambi"` (default), `"haldane"`, `"morgan"` or `"cf"` |
| `...` | Passed to [`qtl::argmax.geno()`](https://rdrr.io/pkg/qtl/man/argmax.geno.html) |

[`quickEst()`](https://drj001.github.io/ASMap/reference/quickEst.md)
sidesteps the slow hidden Markov algorithm of
[`qtl::est.map()`](https://rdrr.io/pkg/qtl/man/est.map.html). Initial
conservative distances are obtained by inverting recombination fractions
from [`est.rf()`](https://rdrr.io/pkg/qtl/man/est.rf.html), then passed
to [`argmax.geno()`](https://rdrr.io/pkg/qtl/man/argmax.geno.html),
where missing allele scores are imputed by the Viterbi algorithm and
distances re-estimated.

------------------------------------------------------------------------

## A worked example

**Click to expand: the condensed barley backcross workflow**

Starting from `mapBCu` - 3,023 markers scored on 326 individuals, with
no linkage groups assigned.
...
Linkage groups beyond ~200 cM suggest genotypes with inflated
recombination counts.
...
The [worked
example](https://drj001.github.io/ASMap/articles/worked-example-refinement.html)
continues from here into fine mapping, combining maps of the same
...
| `mapDHf` | `data.frame` | Doubled haploid (wheat) | 218 | 599 | The same marker set, unconstructed, formatted for [`mstmap.data.frame()`](https://drj001.github.io/ASMap/reference/mstmap.data.frame.md) |
...
These are not lazy-loaded, so an explicit
...
If you find this package useful, please cite it. Run
`citation("ASMap")`, or:

> Taylor, J. & Butler, D. (2017) R Package ASMap: Efficient Genetic
> Linkage Map Construction and Diagnosis. *Journal of Statistical
> Software*, **79**(6), 1-29.
...
Wu, Y., Bhat, P., Close, T.J. & Lonardi, S. (2008) Efficient and
Accurate Construction of Genetic Linkage Maps from the Minimum Spanning
Tree of a Graph. *PLoS Genetics*, **4**(10), e1000212.
...
Found a bug, or have a question about map construction? [Open an
issue](https://github.com/DrJ001/ASMap/issues).

## License

GPL (\>= 2). The bundled MSTmap C++ source code is by Yonghui Wu,
Timothy Close and Stefano Lonardi.
